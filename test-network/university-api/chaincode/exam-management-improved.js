...
class ExamManagementContract extends Contract {
    ...
    async GetExamsByCourse(ctx, courseCode) {
        try {
            ...
            const courseKey = ctx.stub.createCompositeKey('exam~course', [courseCode, '']);
            const iterator = await ctx.stub.getStateByPartialCompositeKey('exam~course', [courseCode]);
            const results = [];
            ...
            console.log(`📚 Found ${results.length} exams for course ${courseCode}`);
