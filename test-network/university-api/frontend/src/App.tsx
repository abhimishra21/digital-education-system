...
const App: React.FC = () => {
  ...
  // Temporary function to clear all data immediately (for testing)
  const clearAllDataImmediately = async () => {
    if (window.confirm('This will delete ALL existing student data. Are you absolutely sure?')) {
      try {
        setIsLoading(true);
        ...
        console.log(`Found ${currentStudents.length} students to delete`);
        
        let deletedCount = 0;
        for (const student of currentStudents) {
          try {
            console.log(`Deleting student: ${student.enrollmentNo}`);
            await apiService.deleteStudentWithHistory(student.enrollmentNo);
            deletedCount++;
            console.log(`Successfully deleted: ${student.enrollmentNo}`);
          } catch (error) {
            console.error(`Failed to delete ${student.enrollmentNo}:`, error);
          }
        }
        ...
        setMessage({ type: 'success', text: `Successfully deleted ${deletedCount} students! Database is now clear.` });
        setTimeout(() => setMessage({ type: 'success', text: '' }), 5000);
        
      } catch (error) {
        console.error('Error clearing data:', error);
        setMessage({ type: 'error', text: 'Error clearing data. Please check console for details.' });
        setTimeout(() => setMessage({ type: 'success', text: '' }), 5000);
      } finally {
        setIsLoading(false);
      }
  ...
  // Add history entry
  const addHistoryEntry = useCallback((studentId: string, action: 'created' | 'updated' | 'deleted', field?: string, oldValue?: string, newValue?: string) => {
    if (!user) return;
    ...
    const historyEntry: StudentHistory = {
      ...
      description: getHistoryDescription(action, field, oldValue, newValue)
    };

    setStudentHistory(prev => [historyEntry, ...prev]);
  }, [user]);

  // Get history description
  const getHistoryDescription = (action: string, field?: string, oldValue?: string, newValue?: string): string => {
    switch (action) {
      case 'created':
  ...
  // Delete student
  const deleteStudent = useCallback(async (enrollmentNo: string): Promise<void> => {
    if (!user) {
      setMessage({ type: 'error', text: 'You must be logged in to perform this action' });
      return;
    }
    ...
    try {
      ...
      // Delete student from blockchain with history tracking
      await apiService.deleteStudentWithHistory(enrollmentNo);
      
      // Reload students from API to ensure consistency
      await loadStudents();
      
      setMessage({ type: 'success', text: 'Student removed successfully from blockchain with history tracking' });
      setTimeout(() => setMessage({ type: 'success', text: '' }), 3000);
    } catch (error) {
      console.error('Failed to delete student:', error);
      setMessage({ 
        type: 'error', 
        text: `Failed to delete student: ${error instanceof Error ? error.message : 'Unknown error'}` 
      });
      setTimeout(() => setMessage({ type: 'success', text: '' }), 5000);
    } finally {
      setIsLoading(false);
    }
  }, [user, isApiConnected, loadStudents]);
  ...
  const saveStudentChanges = async (
    ...
    try {
      ...
      const response = await apiService.updateStudentWithHistory(enrollmentNo, updateData);
  ...
  // Get student history from API
  const getStudentHistory = async (enrollmentNo: string): Promise<StudentHistory[]> => {
    try {
      const response = await apiService.getStudentHistory(enrollmentNo);
  ...
  const clearAllData = async (): Promise<void> => {
    if (!user || user.role !== 'admin') {
      setMessage({ type: 'error', text: 'Only administrators can clear all data' });
      return;
    }
      ...
      try {
        ...
        for (const student of studentsToDelete) {
          try {
            await apiService.deleteStudentWithHistory(student.enrollmentNo);
            deletedCount++;
            console.log(`Deleted student: ${student.enrollmentNo}`);
          } catch (error) {
            console.error(`Failed to delete student ${student.enrollmentNo}:`, error);
          }
        }
        
        setMessage({ type: 'success', text: `Successfully deleted ${deletedCount} out of ${studentsToDelete.length} students!` });
        ...
        setMessage({ type: 'error', text: 'Failed to clear all data. Please try again.' });
        setTimeout(() => setMessage({ type: 'success', text: '' }), 5000);
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <div className="container">
      ...
      {currentPage === 'students' ? (
        ...
        >
          Blockchain-powered student enrollment management with advanced security
        </motion.p>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            {user.role === 'admin' && (
              ...
              >
                {isLoading ? (
                  <LoadingSpinner size={16} text="Clearing..." />
                ) : (
                  <>
                    <Trash2 size={16} />
                    Clear All
            ...
            {user.role === 'admin' && students.length > 0 && (
              <motion.button 
                onClick={clearAllDataImmediately} 
                className="btn btn-danger" 
              ...
              >
                {isLoading ? (
                  <LoadingSpinner size={16} text="Deleting..." />
                ) : (
                  <>
                    <Trash2 size={16} />
                ...
                >
                  <div className="student-list-item-content">
                    ...
                    <div className="list-cell actions-cell">
                      ...
                      <motion.button
                        onClick={() => deleteStudent(student.enrollmentNo)}
                    ...
                    >
                      <div className="expanded-details-grid">
                        ...
                        <div className="detail-item">
                          ...
                          <span className="detail-value">{student.dateOfBirth || 'N/A'}</span>
                        </div>
                        <div className="detail-item">
                        ...
                        <div className="detail-item">
                          ...
                          <span className="detail-value">{student.createdAt || 'N/A'}</span>
                        </div>
                      </div>
                    </motion.div>
                  )}
