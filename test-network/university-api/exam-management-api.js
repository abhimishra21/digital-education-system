    ...
    try {
        ...
        const examRecord = {
            ...
            metadata: {
                ...
                version: '1.0',
                fieldHistory: [], // Track field changes
                customFields: {} // Store any additional fields
            },
        ...
        try {
            ...
            // If endorsement fails, try to provide a helpful error message
            if (submitError.message.includes('ABORTED') || submitError.message.includes('endorse')) {
                throw new Error('ENDORSEMENT_FAILED: Transaction requires endorsement from multiple organizations. This is a network configuration issue.');
            }
            throw submitError;
        }
        ...
        if (error.message.includes('ABORTED') || error.message.includes('endorse')) {
            res.status(400).json({
                success: false,
                message: 'Transaction endorsement failed. This may be due to endorsement policy requirements.',
                error: 'The transaction requires endorsement from multiple organizations. Please ensure all required peers are available.',
...
app.get('/api/exam/:examId', async (req, res) => {
    try {
        ...
        // Pass the parameter as a string to ensure proper formatting
...
app.put('/api/exam/:examId', async (req, res) => {
    try {
        ...
        const result = await contract.submit('UpdateExam', examId, JSON.stringify(updateData), JSON.stringify(fieldChanges));
        const resultString = utf8Decoder.decode(result);
...
// 5. Add Custom Field to Exam
app.post('/api/exam/:examId/custom-field', async (req, res) => {
    try {
        const { examId } = req.params;
        ...
        const result = await contract.submit('AddCustomField', examId, JSON.stringify(customField));
        const resultString = utf8Decoder.decode(result);
        ...
        if (resultString && resultString.trim()) {
            ...
            response = { message: 'Custom field added successfully' };
        }
...
app.get('/api/exam/course/:courseCode', async (req, res) => {
    try {
        ...
        const result = await contract.evaluate('GetExamsByCourse', courseCode);
        const resultString = utf8Decoder.decode(result);
...
// 10. Get Field History for Exam
app.get('/api/exam/:examId/history', async (req, res) => {
    try {
        const { examId } = req.params;
        const result = await contract.evaluate('GetExamFieldHistory', examId);
...
// 11. Delete Exam
app.delete('/api/exam/:examId', async (req, res) => {
    try {
        const { examId } = req.params;
        const result = await contract.submit('DeleteExam', examId);
...
// 12. Bulk Operations
app.post('/api/exam/bulk/create', async (req, res) => {
    try {
        ...
        const result = await contract.submit('BulkCreateExams', JSON.stringify(exams));
        const response = JSON.parse(utf8Decoder.decode(result));
        ...
        res.status(500).json({
            success: false,
            message: 'Failed to bulk create exams',
            error: error.message
        });
    }
});
...
app.get('/api/exam/export/:format', async (req, res) => {
    try {
        ...
        const result = await contract.evaluate('ExportExamData', format, filters || '{}');
...
// 16. Initialize Ledger with Sample Data
