...
// 7. Get All Students (⏱️ with per-call 30s deadline)
app.get('/students', async (req, res) => {
    try {
        // Use a special gateway with a longer timeout
        const client = await newGrpcConnection();
        ...
        const gateway = connect({
            ...
            identity,
            signer,
            evaluateOptions: () => ({ deadline: Date.now() + 30000 }), // 30s timeout
            endorseOptions: () => ({ deadline: Date.now() + 15000 }),
            submitOptions: () => ({ deadline: Date.now() + 5000 }),
            commitStatusOptions: () => ({ deadline: Date.now() + 60000 }),
        });
        ...
        const resultBytes = await contract.evaluateTransaction('GetAllStudents');
        ...
        res.status(500).json({ success: false, message: 'Error getting all students', error: error.message });
    }
});
...
// 17. Search Students with Advanced Filters (⏱️ uses 30s deadline to fetch all)
app.get('/api/students/search', async (req, res) => {
    try {
        ...
        // Fetch all students with a longer deadline to avoid timeouts
        const txn = contract.createTransaction('GetAllStudents');
        const resultBytes = await txn.evaluate({ deadline: Date.now() + 30000 }); // 30s
        const studentsData = utf8Decoder.decode(resultBytes);
