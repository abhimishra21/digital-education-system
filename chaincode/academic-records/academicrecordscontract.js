...
class AcademicRecordsContract extends Contract {
    ...
    async GetAcademicRecordByUniqueFields(ctx, enrollmentNo, cycle, examMonth, examYear, examType, resultHash) {
        try {
            const parts = [enrollmentNo, cycle, examMonth, examYear, examType, resultHash];
            const hasAll = parts.every(v => v !== undefined && v !== null && `${v}`.trim() !== '');
            if (!hasAll) {
                throw new Error('All unique fields must be provided');
            }
            // Prefer the per-semester composite record
            ...
            if (semBytes && semBytes.length > 0) {
                return JSON.parse(semBytes.toString());
            }
            // Fallback to unique pointer to enrollmentNo
            const uniqueKey = ctx.stub.createCompositeKey('academic~unique', parts.map(v => `${v}`));
            const pointer = await ctx.stub.getState(uniqueKey);
            if (pointer && pointer.length > 0) {
                const id = pointer.toString();
                return await this.GetAcademicRecordById(ctx, id);
            }
            return null;
        } catch (error) {
